...
import { getAllOrderPayment, totalOrderAmountPayment } from '../../service/apiPaymentOrder';
...
const OrderPaymentHistory: React.FC = () => {
  ...
  const fetchPaymentAll = async () => {
    try {
      ...
      setPaymentHistory(sortedAppointments.map((appt: PaymentOrderResponse & { isNew?: boolean }) => ({
        ...appt,
                  ...
                  <>
                    {payment.status != 'PENDING' ? (
                      <TableRow key={payment.id} className="hover:bg-gray-100">
                        ...
                        <TableCell>
                          {payment.order.guestName ? payment.order.guestName + " " + payment.id : payment.order.user?.name || "Khách hàng ẩn danh"}
                        </TableCell>
                        <TableCell>
                          {payment.order.orderItems.map((od) => od.product.nameProduct).join(', ')}
                        </TableCell>
